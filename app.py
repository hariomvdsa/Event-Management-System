...
app.config['MYSQL_PASSWORD'] = 'hariom@123'  # Replace with your real MySQL password
app.config['MYSQL_DB'] = 'ems'  # Replace with your DB name
...
def dashboard(username):
    if 'loggedin' in session and username == session['username']:
        ...
        # First Query: Get personal + contact info
        cursor.execute(
            '''
        ...
        # Second Query: Count events booked
        cursor.execute(
            '''
        ...
        # Third Query: Fetch event details
        cursor.execute(
            '''
            SELECT e.*, b.* 
            FROM event e NATURAL JOIN books b 
            WHERE e.eid IN (
                SELECT eid FROM books 
                WHERE uid = (
                    SELECT uid FROM users WHERE username = %s
                )
            )
...
def book_event(eventname: str):
    if request.method == 'POST':
        ...
        # Determine event type and participants
        ...
        tier_num = {'tier1': 1, 'tier2': 2, 'tier3': 3, 'tier4': 4}[tier]

        cursor.execute('''INSERT INTO event (etype, edate, etier, ecost, evenue, emax_people, especial, status)
                          VALUES (%s, %s, %s, %s, %s, %s, %s, 'pending')''',
                       (etype, date, tier_num, cost, venue, max_people, requests_msg))
        ...
        if user_result:
            send_email(
                user_result['email'],
                f"📅 Event Booking Confirmed - {etype}",
                f"Hi {session['username']},\n\nYour event ({etype}) has been successfully booked for {date} at {venue}.\n\nTotal Cost: ₹{cost}\n\nThanks,\nTeam EMS"
            )
        ...
        for admin in cursor.fetchall():
            send_email(
                admin['email'],
                "🔔 New Event Booking Alert",
                f"A new {etype} event was booked by {session['username']} for {date} at {venue}.\nTier: {tier}\nGuests: {max_people}\nCost: ₹{cost}"
            )
...
def personal():
    if 'loggedin' in session:
        ...
        if request.method == "POST":
            ...
            DOB = request.form.get('dob')  # safer access
